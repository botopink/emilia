...
3. **`Token` enum** — the typed authored surface (see `tokens.bp`).
   ...
   Effect + the 82 modifier variants of front 34, each carrying a nested
...
`case`), and nothing about emission. There are **82** of them, 82 variants and
...
- `botopink test` inside `modules/emilia/` (never at the root — the umbrella
  refuses) runs every module's in-file `test {}` blocks, **269/269** on
  commonJS and on erlang: 6 (`spacing.bp`) + 37 (`theme.bp`) + 45
  (`output.bp`) + 181 (`emilia.bp`). `emilia.bp`'s 181 are front 56's 33
  (below) plus front 33's 81 plus front 35's 31 plus front 34's 36 — two per
...
| 1.0.10-beta front 34 — modifiers | steps 1-6 — the variant table in `tokens.bp` + `emilia.bp` under the front's banner; 82 modifiers; steps 1-6. One `Variant`-returning fn per name and no wrapping logic: front 56's `nestVariant` applies them |
