...
3. **`Token` enum** — the typed authored surface (see `tokens.bp`).
   ...
   Effect + the 64 modifier variants of front 34, each carrying a nested
...
`case`), and nothing about emission. There are **64** of them, 64 variants and
...
- `botopink test` inside `modules/emilia/` (never at the root — the umbrella
  refuses) runs every module's in-file `test {}` blocks, **261/261** on
  commonJS and on erlang: 6 (`spacing.bp`) + 37 (`theme.bp`) + 45
  (`output.bp`) + 173 (`emilia.bp`). `emilia.bp`'s 173 are front 56's 33
  (below) plus front 33's 81 plus front 35's 31 plus front 34's 28 — two per
...
| 1.0.10-beta front 34 — modifiers | steps 1-5 — the variant table in `tokens.bp` + `emilia.bp` under the front's banner; 64 modifiers; steps 1-5. One `Variant`-returning fn per name and no wrapping logic: front 56's `nestVariant` applies them |
